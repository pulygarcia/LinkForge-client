...
type LinkForgeInputProps = {
    item: LinkForgeLink
    handleInputChange: (e:React.ChangeEvent<HTMLInputElement>) => void;
    handleEnableLink:(socialNetwork:string) => void;
}

export default function LinkForgeInput({ item, handleInputChange, handleEnableLink } : LinkForgeInputProps) {
    return (
        <div className="bg-white p-3 rounded-md flex items-center gap-4">
            ...
            <input 
                type="text" 
                className="flex-1 border border-gray-100 rounded-md"
                value={item.url}
                name={item.name}
